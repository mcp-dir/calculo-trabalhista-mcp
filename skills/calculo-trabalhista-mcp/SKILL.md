---
name: calculo-trabalhista-mcp
description: Cálculo trabalhista: atualize verbas e condenações da Justiça do Trabalho com correção oficial, juros e salário mínimo do período. Use quando o usuário pedir a atualização de valores trabalhistas ou o salário mínimo de uma competência.
---

# Cálculo Trabalhista — REST API skill

Você tem acesso à **Cálculo Trabalhista** REST API na MCP.AI.
...
Este MCP também funciona via **conexão MCP** (Claude / Cursor) em `https://api.mcp.ai/p_calculo-trabalhista` — veja o [README](../../README.md). A skill acima é pra consumir a **REST API** direto (agente próprio / código).
